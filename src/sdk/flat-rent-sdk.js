...
export class FlatRentSdk {
   ...
   */
  get(id) {
    const flat = this.database.find((item) => item.id === id);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (parameters.priceLimit != null) {
          flats = flats.filter((flat) => flat.price <= parameters.priceLimit);
        }

        const dateRange = this._generateDateRange(parameters.checkInDate, parameters.checkOutDate);
        flats = flats.filter((flat) => this._areAllDatesAvailable(flat, dateRange));

        flats = flats.map((flat) => this._formatFlatObject(flat, dateRange.length - 1));
  ...
  book(flatId, checkInDate, checkOutDate) {
    return new Promise((resolve, reject) => {
      try {
        ...
        const bookedDates = datesToBook.map((date) => date.getTime());
  ...
  _areAllDatesAvailable(flat, dateRange) {
    return dateRange.every((date) => !flat.bookedDates.includes(date.getTime()));
  }

  _formatFlatObject(flat, nightNumber) {
    const formattedFlat = { ...flat };

    formattedFlat.photos = formattedFlat.photos.map((photoUrl) => `./img/${photoUrl}`);
